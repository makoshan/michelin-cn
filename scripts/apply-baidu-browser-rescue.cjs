...
const decisionPath = path.join(root, `${city}-pi-baidu-browser-decisions.md`);
const overridePath = path.join(root, `${city}-baidu-browser-poi-overrides.json`);
...
function parseAccepted(markdown) {
  ...
  return [...acceptedSection.matchAll(/^- \*\*(\d+)\*\* — \*\*(.*?)\*\*：(.+)$/gm)].map((match) => ({
    id: Number(match[1]),
    name: match[2].replace(/^建议高德搜索\/写入中文名：/, "").trim(),
    reason: match[3].trim(),
  }));
}
...
for (const decision of decisions) {
  ...
  if (!restaurant || !poi) {
    skipped.push({ id: decision.id, name: decision.name, reason: "no confirmed Amap override" });
    continue;
  }
  ...
  restaurant.aiPoiReason = `Pi/Kimi rescued rejected via Baidu browser search: ${decision.reason}`;
...
console.log(`${shouldApply ? "Applied" : "Dry run"} ${applied.length} Baidu-browser rescues.`);
